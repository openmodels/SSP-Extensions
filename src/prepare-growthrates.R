...
for(scenario in paste0("ssp", 1:5)) {
  ...
  df_pop <- read.csv(paste0("../results/", scenario, "-pop.csv"), stringsAsFactors = F) %>% filter(year %in% years) %>%
  ...
  # write into csv file
  write_csv(df_pop, paste0("../results/", scenario, "_pop_rate.csv"))

  # repeat for pcGDP
  df_gdppc <- read.csv(paste0("../results/", scenario, "-gdppc.csv"), stringsAsFactors = F) %>% filter(year %in% years) %>%
    select(year, region, rate) %>% spread(region, rate) %>%
    select(year, EU, US, OT, EE, CA, IA, AF, LA) # change order of the columns according to PAGE input files
  ...
  write_csv(df_gdp, paste0("../results/", scenario, "_gdp_rate.csv"))
